...
OWNER_GPU_MEM_UTIL=${OWNER_GPU_MEM_UTIL:-"0.18"}
CONSUMER_GPU_MEM_UTIL=${CONSUMER_GPU_MEM_UTIL:-"0.06"}
OUT_JSON=${OUT_JSON:-"/tmp/deepmath_conc10_only.json"}
...
cleanup() {
  # kill known background services
  ...
  # kill all GPU compute processes shown by nvidia-smi
  if nvidia-smi --query-compute-apps=pid --format=csv,noheader >/tmp/_conc10_gpu_pids.txt 2>/dev/null; then
    cat /tmp/_conc10_gpu_pids.txt | tr -d ' ' | sed '/^$/d' \
      | while read -r p; do
...
echo "[7/7] summarize throughput"
python - <<'PY'
import json
import re

report_path = "/tmp/deepmath_conc10_only.json"
...
echo "  launch log: /tmp/launch4_conc10.log"
